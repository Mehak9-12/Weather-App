...
function CityDropdown({ apiKey, onSelect }) {
  ...
  // Fetch API results when debouncedQuery changes
  useEffect(() => {
    // If query is too short, we don't hit the API (we show popular cities instead)
  ...
  const handleClear = () => {
    setQuery("");
    setResults([]);
    // We keep it open so they can see popular cities immediately
    setOpen(true);
  };
      ...
      {open && (
        <div className="absolute w-full mt-2 bg-white border border-slate-100 rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="max-h-60 overflow-y-auto custom-scrollbar">
            ...
            {/* Default Popular Cities (When query is short) */}
            ...
            {query.length >= 3 && !loading && results.length > 0 && (
              <div>
                {results.map((city) => (
                   <div
                    key={`${city.id}-${city.lat}`} // using combined key to be safe
